...
TOKEN_QUEUE_SIZE = 1000

# Shared token queue for better throughput
...
def send_tokens_batch(tokens):
    """Send multiple tokens in one batch request"""
    if not tokens:
        return 0
    try:
        payload = {"tokens": tokens}
        r = requests.post(TOKEN_SAVE_ENDPOINT, json=payload, timeout=10)
...
def get_global_model():
    global _model_state, _model_usage
...
def get_sift_detector():
    ...
    if _sift_detector is None:
        with _sift_lock:
            if _sift_detector is None:
                try:
                    _sift_detector = cv2.SIFT_create(nfeatures=30, contrastThreshold=0.08)
                    logger.info("Using cv2.SIFT_create()")
                except AttributeError:
                    try:
                        _sift_detector = cv2.xfeatures2d.SIFT_create(nfeatures=30, contrastThreshold=0.08)
        ...
        try:
            ...
            if resp_json.get('result') == True:
                ...
                if validate_decoded and len(validate_decoded.strip()) > 10:
                    # Add to global token queue
                    global token_queue, token_counter
                    with token_lock:
                        token_counter += 1
                        try:
                            token_queue.put_nowait(validate_decoded)
                        except queue.Full:
                            pass
                    
                    # Also send to server
                    send_token_to_server(validate_decoded)
                    self.save_token_locally(validate_decoded)
                    logger.success(f'T-{self.thread_id} SUCCESS: {validate_decoded[:40]}...')
                    return True
                else:
...
# Global token collector
token_batch_queue = queue.Queue()
batch_processor_running = False

def process_token_batch_worker():
    """Background worker to batch-process tokens"""
    ...
    while batch_processor_running:
        try:
            # Try to get a token with timeout
            try:
            ...
            if len(batch) >= 10 or (len(batch) > 0 and current_time - last_flush > 2.0):
                if batch:
                    sent = send_tokens_batch(batch)
                    if sent < len(batch):
                        # Save unsent tokens locally
                        for token in batch[sent:]:
                            with file_lock:
                                with open(TOKEN_OUTPUT_FILE, 'a') as f:
                                    f.write(f"{token}\n")
                    batch = []
...
def start_batch_processor():
    ...
    global batch_processor_running
    if not batch_processor_running:
        thread = threading.Thread(target=process_token_batch_worker, daemon=True)
        thread.start()
        logger.info("Batch processor started")

def worker_thread(thread_id, config):
    """Optimized worker with improved session reuse and backoff"""
    consecutive_failures = 0
    success_count = 0
    last_recreate = time.time()
    
    # Create solver instance
    solver_instance = Dun163(
        id_=config['ID_'],
    ...
    while True:
        try:
            ...
            if consecutive_failures > 0:
                backoff = min(consecutive_failures * 0.5, 5.0)
                time.sleep(backoff)
            
            # Refresh session periodically
            if consecutive_failures % 20 == 0 and consecutive_failures > 0:
                logger.info(f"T-{thread_id} | Refreshing solver instance...")
                solver_instance = Dun163(
                    id_=config['ID_'],
                    referer=config['REFERER'],
                    fp_h=config['FP_H'],
            ...
            success = solver_instance.run(attempt_num=success_count + 1)
            
            if success:
                success_count += 1
                consecutive_failures = 0
                # Small delay between successes to avoid rate limiting
                time.sleep(random.uniform(0.2, 0.5))
            else:
                consecutive_failures += 1
                logger.warning(f"T-{thread_id} | Failed ({consecutive_failures} in a row)")
...
def main():
    ...
    # Configuration
    NUM_THREADS = int(os.environ.get('CN31_WORKERS', MAX_WORKERS))
